<?php
class Datestatistics_model extends CI_Model {
	...
	public function sumcompany(){
		$company_sql="select company_name from amp_company";
		$company_query = $this->db->query($company_sql);
		//echo '<pre>';var_dump($company_query);echo '</pre>';exit;
	
		return $company_query->result_array;
		//echo '<pre>';var_dump($data['COMPANY']);echo '</pre>';exit;
	}
	public function sumproject(){
		$project_sql="select project from amp_project";
		$project_query = $this->db->query($project_sql);
		return $project_query->result_array;
	}
	public function sumbuilding(){
		$building_sql="select building from amp_building";
		$building_query = $this->db->query($building_sql);
		return $building_query->result_array;
	}
	public function sumfloor(){
		$floor_sql="select floor from amp_floor";
		$floor_query = $this->db->query($floor_sql);
		return $floor_query->result_array;
		//$alldata[]=$dataval;
